...
int main (int argc, char *argv[]) {
    ...
    int msqid = msgget(msgKey, IPC_CREAT | 0600);
    ...
    scanf("%s", buffer);
    order.mtype = strcmp(buffer, "y") == 0 ? 1 : 2; 
    ...
    len = sizeof(order) - sizeof(order.mtype);

    if(msgsnd(msqid, (void *) &order, len, 0) == -1) errExit("MSGSND Failed"); 
